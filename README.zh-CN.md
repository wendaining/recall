...
历史 TUI 会加载全部匹配记录，没有条数上限。
在 **Retention** 分类中，对 **Output retention** 按 Enter 可输入保留天数
（0 表示永久保留输出）；对 **Auto prune** 按 Space 或 Enter 可切换自动清理。
自动清理在打开历史 TUI 时删除过期输出，命令记录仍会保留；保存设置不会立即清理。
保留天数为 0 与关闭自动清理对手动清理的影响不同，详见[输出保留与清理设置](docs/retention.md)。
